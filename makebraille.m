function [imf,imb] = makebraille(dots,charin)
...
%
%   INPUT: dots must be list of integers 1-6, charin is the character
%   OUTPUT: 2 images (uint8) showing braille dots with character at the
%   bottom
%   i.e.
%
...
for z=1:length(dots)
   ...
   elseif dots(z) == 6
       ...
       disp('WARNING: invalid dot requested...')
   end
end
ylim([-0.2 4])
xlim([0.5 2])
...
set(gca,'YTick',[]);
set(gca,'XTick',[]);
rectangle('position',[0.5 -0.2 1.5 4.2],'edgecolor','k','LineWidth',4)
text(1,0.3,charin,'FontSize',50);
